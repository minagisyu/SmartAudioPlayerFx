...
//	[Require(typeof(MediaItemFilterManager))]
	sealed class MediaDBViewManager : IDisposable
	{
		...
		{
			App.Services.GetInstance<LogManager>().AddDebugLog($"Call GetOrCreate: filepath={filepath}");
		...
		{
			App.Services.GetInstance<LogManager>().AddDebugLog($"Call LoadDBItems: path={path}");
			...
			_LoadDBItems_Task = Task.Run(() =>
			{
				...
				sw.Stop();
				App.Services.GetInstance<LogManager>().AddDebugLog($" **LoadDBItems({Items.Count}items): {sw.ElapsedMilliseconds}ms");
			})
			.ContinueWith(_ =>
			{
				CollectFiles(path);
				...
				if (itemsLoaded_ev != null)
					itemsLoaded_ev();
				App.Services.GetInstance<LogManager>().AddDebugLog(" **LoadDBItems(full-complete)");
			});
		}
		...
		{
			App.Services.GetInstance<LogManager>().AddDebugLog($"Call RevalidateItems: path={path}");
			...
			_revalidateItems_Task = Task.Run(() =>
			{
				...
				App.Services.GetInstance<LogManager>().AddDebugLog($" ..RevalidateTask: items version:{Items.Version}");
				var sw = Stopwatch.StartNew();

				// phase remove: 既存の項目に対して、Validate()が通らないものを削除
				App.Services.GetInstance<LogManager>().AddDebugLog($" ..RevalidateTask-phase1: version:{Items.Version}");
				Items.GetLatest()
					.TakeWhile(_ => ct.IsCancellationRequested == false)
					.AsParallel()
				...
				sw.Stop();
				App.Services.GetInstance<LogManager>().AddDebugLog($" **RevalidateItems: {sw.ElapsedMilliseconds}ms");
			})
			.ContinueWith(_ =>
			{
				...
				if (itemsLoaded_ev != null)
					itemsLoaded_ev();
				App.Services.GetInstance<LogManager>().AddDebugLog(" **RevalidateItems(full-complete)");
			});
		}
		...
		{
			App.Services.GetInstance<LogManager>().AddDebugLog($"Call CollectFiles: path={path}");
			...
			_collectFiles_Task = Task.Run(() =>
			{
				...
				App.Services.GetInstance<LogManager>().AddDebugLog(" - CollectItems start.");
				...
				using (fsw)
				{
					...
					sw.Stop();
					App.Services.GetInstance<LogManager>().AddDebugLog($" - CollectFiles phase1({add_count}items add, elapsed {sw.ElapsedMilliseconds}ms)");
					if (notify != null)
						notify(string.Empty);
					...
					sw.Stop();
					App.Services.GetInstance<LogManager>().AddDebugLog($" - CollectFiles phase2({updateList.Count}items update, elapsed {sw.ElapsedMilliseconds}ms)");
					...
					sw.Stop();
					App.Services.GetInstance<LogManager>().AddDebugLog($" - CollectFiles phase3({updateList.Count}items update, ellapsed {sw.ElapsedMilliseconds}ms");
					...
					while (ct.IsCancellationRequested == false)
					{
						...
						App.Services.GetInstance<LogManager>().AddDebugLog("FSW-Notify, RaiseRescan.");
						Task.Run(() => CollectFiles(path));
						break;
					}
				}
				App.Services.GetInstance<LogManager>().AddDebugLog(" - CollectFiles finished.");
			});
