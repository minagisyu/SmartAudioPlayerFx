...
namespace SmartAudioPlayerFx.Windows
{
	...
	{
		// TODO:
		// ・キュー式に変更して1つずつ処理するように変更
		// ・Get()の戻り値をIObservableに
		// ・Suspress/Resume(一時停止/再開)を可能にする (ディスクアクセスやCPU使用率の関係)
		// ・FilesSource内部利用から、サービスとしての明示的な利用に変更する
		static ManualResetEventSlim getlock = new ManualResetEventSlim(true);
		...
		public static MediaTagInfo Get(string filePath)
		{
			getlock.Wait();
			getlock.Reset();
			var result = new MediaTagInfo_Mp3infp(filePath);
			getlock.Set();
			return result;
		}
		...
		sealed class MediaTagInfo_Mp3infp : MediaTagInfo
		{
			...
			{
				LogService.AddDebugLog("Call static ctor");
				try
				{
					var version = Mp3infp.GetVer();
					disabled = (version < 0x253);
					LogService.AddDebugLog(" - mp3infp version: {0}", version);
				}
				catch (Exception)
				{
					LogService.AddDebugLog(" - GetVer() call error");
					disabled = true;
				}
				LogService.AddDebugLog(" - tagservice_is_disable={0}", disabled);
			}
			...
			{
				LogService.AddDebugLog("Call ctor: filepath={0}", filepath);
				FilePath = filepath;
					...
					lock (sync)
					{
						...
						if (Mp3infp.Load(IntPtr.Zero, FilePath) == 0)
						{
							switch (Mp3infp.GetFileType())
							{
								...
								default:
									IsTagLoaded = false;
									LogService.AddDebugLog(" - Mp3infp.GetFileType() == unknown");
									break;
							}
						...
						{
							IsTagLoaded = false;
							LogService.AddDebugLog(" - Mp3infp.Load() failed.");
						}
					}
				}
				catch (Exception e)
				{
					IsTagLoaded = false;
					LogService.AddErrorLog("Mp3infp.Load() failed: path=" + filepath, e);
				}
			}
			...
			void CollectByMP3()
			{
				LogService.AddDebugLog("Call CollectByMP3");
				var tagtype = Mp3infp.Mp3_GetTagType();
				LogService.AddDebugLog(" - Mp3infp.Mp3_GetTagType() == {0}", tagtype);
			...
			void CollectByWav()
			{
				LogService.AddDebugLog("Call CollectByWav");
			...
			void CollectByAvi()
			{
				LogService.AddDebugLog("Call CollectByAvi");
				Mp3infp.GetValue("INAM", out Title);
				Mp3infp.GetValue("IART", out Artist);
				Mp3infp.GetValue("ICMT", out Comment);
			}
			...
			void CollectByVQF()
			{
				LogService.AddDebugLog("Call CollectByVQF");
				Mp3infp.GetValue("INAM", out Title);
				Mp3infp.GetValue("IART", out Artist);
				Mp3infp.GetValue("ICMT", out Comment);
			}
			...
			void CollectByWMA()
			{
				LogService.AddDebugLog("Call CollectByWMA");
				Mp3infp.GetValue("INAM", out Title);
				Mp3infp.GetValue("IART", out Artist);
				Mp3infp.GetValue("IPRD", out Album);
				Mp3infp.GetValue("ICMT", out Comment);
			...
			void CollectByOGG()
			{
				LogService.AddDebugLog("Call CollectByOGG");
				Mp3infp.GetValue("INAM", out Title);
				Mp3infp.GetValue("IART", out Artist);
				Mp3infp.GetValue("IPRD", out Album);
				Mp3infp.GetValue("ICMT", out Comment);
			...
			void CollectByAPE()
			{
				LogService.AddDebugLog("Call CollectByAPE");
				Mp3infp.GetValue("INAM", out Title);
				Mp3infp.GetValue("IART", out Artist);
				Mp3infp.GetValue("IPRD", out Album);
				Mp3infp.GetValue("ICMT", out Comment);
			...
			void CollectByMpeg4()
			{
				LogService.AddDebugLog("Call CollectByMpeg4");
				Mp3infp.GetValue("INAM", out Title);
				Mp3infp.GetValue("IART", out Artist);
				Mp3infp.GetValue("IPRD", out Album);
				Mp3infp.GetValue("ICMT", out Comment);
