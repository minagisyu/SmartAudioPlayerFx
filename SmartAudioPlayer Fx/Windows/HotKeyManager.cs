...
namespace SmartAudioPlayerFx.Windows
{
	...
	sealed class HotKeyManager
	{
		...
		{
			LogService.AddDebugLog("Call SetHotKey: key={0}", key);
		...
		{
			LogService.AddDebugLog("Call RemoveHotKey: key={0}", key);
			Action v;
		...
		{
			LogService.AddDebugLog("Call RemoveAll");
		...
		void hook_KeyboardHooked(object sender, KeyboardHook.HookedEventArgs e)
		{
			...
			LogService.AddDebugLog(" **KeyboardHooked: LastDownKey={0}", LastDownKey);
