...
namespace SmartAudioPlayerFx.UI.Options
{
	sealed partial class OptionDialog_MediaList : OptionPage
	{
		...
		{
			JukeboxService.AllItems.MediaItemFilteringManager.AcceptExtensions
			...
			JukeboxService.AllItems.MediaItemFilteringManager.IgnoreWords
		...
		public override void Save()
		{
			var exts = GetDataGridViewItems(accept_exts)
				...
				.Select(i => new MediaItemFilteringManager.AcceptExtension(i.Key, i.Value));
			JukeboxService.AllItems.MediaItemFilteringManager.SetAcceptExtensions(exts);
			var words = GetDataGridViewItems(ignore_words)
				.Where(i => !string.IsNullOrWhiteSpace(i.Value))
				.Where(i => !ValueCheck_HasValue(i.Value))
				.Select(i => new MediaItemFilteringManager.IgnoreWord(i.Key, i.Value));
