...
namespace SmartAudioPlayerFx.UI.Views
{
	sealed partial class MediaListWindow_Resources : ResourceDictionary
	{
		...
		void MenuItem_ListItem_IgnoreFile_Click(object sender, RoutedEventArgs e)
		{
			...
			var item = menuitem.DataContext as MediaItemViewModel;
			if (item == null) return;
			var list = new List<MediaItemFilteringManager.IgnoreWord>(JukeboxService.AllItems.MediaItemFilteringManager.IgnoreWords);
			list.RemoveAll(i => string.Equals(i.Word, item.FilePath, StringComparison.CurrentCultureIgnoreCase));
			list.Add(new MediaItemFilteringManager.IgnoreWord(true, item.FilePath));
			JukeboxService.AllItems.MediaItemFilteringManager.SetIgnoreWords(list);
			JukeboxService.AllItems.ReValidate_Items();
		}
		...
		void MenuItem_TreeItem_IgnoreDir_Click(object sender, RoutedEventArgs e)
		{
			...
			list.Add(new MediaItemFilteringManager.IgnoreWord(true, item.BasePath));
			JukeboxService.AllItems.MediaItemFilteringManager.SetIgnoreWords(list);
			JukeboxService.AllItems.ReValidate_Items();
		}
		...
		void CheckBox_Click(object sender, RoutedEventArgs e)
		{
			...
			if (vm == null) return;
			Task.Factory.StartNew(() => MediaDBService.Update(vm.Item, _ => _.IsFavorite));
		}
