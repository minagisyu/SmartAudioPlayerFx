...
#region Test
#if DEBUG
namespace SmartAudioPlayerFx.Player.Test
{
	...
	public class AudioPlayerTest
	{
		...
		public void PlayFrom()
		{
			...
			var source = Path.Combine("v:", "!!nothing-file!!.wav");
			var player = new SmartAudioPlayerFx.Player.AudioPlayer();
			player.PlayFrom(source, true, null, null);
			player.CurrentOpenedPath.Is((string)null, "open NOT exists file");
			player.Close();

			// open exists file
			source = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Windows), "media", "tada.wav");
			player = new SmartAudioPlayerFx.Player.AudioPlayer();
			player.PlayFrom(source, true, null, null);
			player.CurrentOpenedPath.Is(source, "open exists file");

			// test other method
			player.PlayFrom(source, false, TimeSpan.FromSeconds(1), null);
			player.Close();
		}
