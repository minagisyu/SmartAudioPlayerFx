...
namespace SmartAudioPlayerFx.ViewModels
{
	...
	sealed class MediaTreeViewSource
	{
		...
		public MediaTreeViewSource()
		{
			...
			JukeboxService.ViewFocus.ViewItemsChanged += () =>
			{
				UIService.UIThreadInvoke(() => ResetMediaTreeItems());
			};
			// ViewItems更新処理追随
			JukeboxService.ViewFocus.ViewItemChanged += e =>
			{
				if (media_tree_root == null) return;
				...
				if (e.ChangedType == MediaDBView.MediaItemChangedType.Add)
					UIService.UIThreadInvoke(() =>
					{
						if (media_tree_root != null)
							media_tree_root.ApplyByDirectoryPath(e.DirectoryName);
					});
				if (e.ChangedType == MediaDBView.MediaItemChangedType.Remove)
					UIService.UIThreadInvoke(() =>
					{
						if(media_tree_root != null)
							media_tree_root.RemoveByDirectoryPath(e.DirectoryName);
					});
			};
		}
		...
		{
			LogService.AddDebugLog("Call ResetMediaTreeItems");
			if (media_tree_root != null)
			{
			...
			if (string.IsNullOrWhiteSpace(JukeboxService.ViewFocus.ViewFocusPath) == false)
			{
				...
				JukeboxService.ViewFocus.ViewItems.Values
					.GroupBy(i => i.GetFilePathDir())
					.Run(g => media_tree_root.ApplyByDirectoryPath(g.Key));
				sw.Stop();
				LogService.AddDebugLog(" - tree creation: {0}ms", sw.ElapsedMilliseconds);
			}
		}
