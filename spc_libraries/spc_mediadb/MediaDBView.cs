...
{
	[Require(typeof(MediaDB))]
	[Require(typeof(Preferences))]
	[Require(typeof(MediaItemFilter))]
	public sealed class MediaDBView : IDisposable
	{
		#region ctor
		...
		readonly MediaDB _mediaDB;
		readonly MediaItemFilter _mediaItemFilter;

		public MediaDBView(Preferences preferences, MediaDB mediaDB, MediaItemFilter mediaItemFilter)
		{
			...
			_mediaDB = mediaDB;
			_mediaItemFilter = mediaItemFilter;

			// Preferences
			preferences.PlayerSettings
				.Subscribe(x => LoadPreferences(x))
				.AddTo(_disposables);
			preferences.SerializeRequestAsObservable()
				.Subscribe(_ => SavePreferences(preferences.PlayerSettings.Value))
				.AddTo(_disposables);

			// MediaItemFilter
			// 最後の通知から500ms後にItemsの再検証
			_mediaItemFilter.PropertyChangedAsObservable()
		...
		public void RaiseDBUpdate(MediaItem item, params Expression<Func<MediaItem, object>>[] columns)
		{
			...
			using (var dbaction = _mediaDB.BeginTransaction())
			{
				dbaction.Update(new[] { item }, columns)
					.AsParallel()
					.Where(x => _mediaItemFilter.Validate(x))
		...
		async Task LoadDBItems(string path)
		{
			...
			await Task.Run(() =>
			{
				...
				_mediaDB.GetFromFilePath_ExistsOnly(path)
		...
		async Task RevalidateItems(string path)
		{
			...
			await Task.Run(() =>
			{
				...
				Items.GetLatest()
					.TakeWhile(_ => ct.IsCancellationRequested == false)
					.AsParallel()
					.Where(x => _mediaItemFilter.Validate(x) == false)
				...
				using (fsw)
				{
					...
					foreach (var x in FileSystemUtil.EnumerateAllFiles(path, ct))
					{
						...
						var xs = x.Files
							...
							.Where(y => _mediaItemFilter.Validate(y));
						using (var dbaction = _mediaDB.BeginTransaction())
						{
							dbaction.Insert(xs)
								.AsParallel()
	...
	public static class MediaDBViewManagerExtensions
	{
		public static IObservable<string> ItemsCollectingAsObservable(this MediaDBView manager)
		{
			return Observable.FromEvent<string>(v => manager.ItemsCollecting += v, v => manager.ItemsCollecting -= v);
		}
		public static IObservable<Unit> ItemCollect_CoreScanFinishedAsObservable(this MediaDBView manager)
		{
			return Observable.FromEvent(v => manager.ItemCollect_CoreScanFinished += v, v => manager.ItemCollect_CoreScanFinished -= v);
		}
		public static IObservable<Unit> ItemCollect_ScanFinishedAsObservable(this MediaDBView manager)
		{
			return Observable.FromEvent(v => manager.ItemCollect_ScanFinished += v, v => manager.ItemCollect_ScanFinished -= v);
		}
		public static IObservable<Unit> ItemsLoadedAsObservable(this MediaDBView manager)
		{
