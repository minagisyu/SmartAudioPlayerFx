	...
	sealed class JukeboxManager : IDisposable
	{
		...
		public void SelectNext(bool skipMode, bool by_error = false)
		{
			...
			if (items.Length == 0)
			{
				...
				{
					// 再生＆選択不可能なのでnull
					App.Services.GetInstance<LogManager>().AddDebugLog("SelectNext: no items. (by error)");
					CurrentMedia.Value = null;
				}
				else
				{
					App.Services.GetInstance<LogManager>().AddDebugLog("SelectNext: no items.");
					CurrentMedia.Value = CurrentMedia.Value;
				}
				return;
			}
			...
			if (items.Length == 1)
			{
				...
				if (by_error)
				{
					App.Services.GetInstance<LogManager>().AddDebugLog("SelectNext: one items, by_error==true, set CurrenyMedia = null.");
					CurrentMedia.Value = null;
					return;
				}

				App.Services.GetInstance<LogManager>().AddDebugLog("SelectNext: one items.");
